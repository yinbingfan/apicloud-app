...
define('slide', function(require, exports, module) {
    "use strict";
    seajs.importStyle('.slide{ position:relative;overflow:hidden;background: url(../../../res/img/ic_slide_default@3x.png) no-repeat;background-size: contain;}\
        .slide img[slide-src]{opacity:0}\
        .slide .slide-title {position: absolute;bottom:0;left:0; width: 94%;background-image: linear-gradient(-180deg, rgba(0, 0, 0, 0.00) 0%, rgba(0, 0, 0, 0.42) 31%, rgba(0, 0, 0, 0.60) 55%, #121111 100%);padding:1rem 3%;height:3.8rem;}\
        .slide .slide-title .banner-tit{ display: inline-block;width: 100%; font-weight: bold;font-size:1.6rem;color: #FFFFFF;letter-spacing: -0.45px;line-height:5rem;overflow:hidden;text-overflow: ellipsis;text-align: center;-webkit-line-clamp: 1;white-space: nowrap;word-break:break-all;}\
        ...
        .slide_nav a{display:inline-block;width:0.7rem;height:0.7rem;border-radius:0.7rem;overflow:hidden;text-indent:-99px;background:rgba(255,255,255,0.4);margin:0rem 0.2rem 3rem 0.2rem;cursor:pointer}\
        .slide_nav .on{background:#fff;}\
    ...
    var Slide = function(config) {
        ...
        if ($.isArray(opt.data)) {
            ...
            $.each(opt.data, function(i, e) {
                ...
                slideNode += ('<li' + attribute + '><span class="slide-title"><span class="banner-tit">' + e.title + '</span></span><a href="' + (e.link || 'javascript:;') + '"><img ' + (opt.lazyload ? 'slide-src="' : 'src="') + (e.src || '') + '" alt="' + (e.alt || '') + '"></a></li>')
            });
            slideNode += '</ul>';
        }
