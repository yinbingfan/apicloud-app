...
define('option-list', function(require, exports, module) {
    ...
    var item_temp =
        //  '<li class="item item-right-options scrollSelectItem ${className}">\
        //         <div class="item-content scrollSelectContent"> ${title|raw} </div>\
        //     </li>'
        '<li class="item item-right-options scrollSelectItem ${className}">\
        <div id="datalist"  style="line-height:4.2rem;text-align:center;background: #EEF1F6;font-size:1.3rem;color: rgba(32,32,32,.5);letter-spacing: 0.13px;">${date}</div>\
        <div class="row news item-content scrollSelectContent" data-guid="${item.guid}" data-url="${storeUrl}" data-id=${id} style="border-bottom:${border}">\
          <!-- 左侧 -->\
            <div class="span-4">\
                <img src="${listImgUrl}" alt="">\
                <img style="position:absolute;width:2.5rem;height:2.5rem;left:1.9rem;top:1.8rem;display:${displayMedia}" src="${mutliMedia}" alt="">\
              </div>\
              <div class="span-left" style="position:relative;">\
  <!-- 新闻标题 -->\
    <div class="news-title" id="title">${title}</div>\
    <div style="position:absolute;bottom:0px;">\
        <div class="span-12" style="font-size:1.1rem;line-height:20px;color:#b4b3b3;">\
            <span>${newsTime}</span> | <span id="media">${name}</span>\
        </div>\
      </div>\
            </div>\
            </div>\
        ...
        button_temp = '<div class="item-options scrollSelectOptions">\
            <!-- for: ${buttons} as ${btn}, ${index} --><div class="btn ${btn.className}" data-index="${index}" data-id="${id}"> ${btn.text} </div><!-- /for -->\
        </div>',
        ...
        buildListview: function(newData) {
            ...
            if (!self.init) {
                var startX = 0,
                    ...
                    flag = 0; //是否已经计算过角度
                    console.warn(self);
                    console.warn(self.ele);
                    console.warn(self.ele.get(0));
